...
class BluetoothDeviceConnection private constructor(val context: Context, messenger: BinaryMessenger, val index: Int, private val deviceWrapper: DeviceWrapper, private val safeMethod: Boolean = false) : MethodChannel.MethodCallHandler {
    ...
    companion object {
        ...
        fun buildBluetoothDeviceConnection(context: Context, messenger: BinaryMessenger, device: DeviceWrapper, safe: Boolean): BluetoothDeviceConnection {
            val index = makeIndex()
            val bluetoothDeviceConnection = BluetoothDeviceConnection(context, messenger, index, device, safe)
    ...
    private val channel = MethodChannel(messenger, "top.kikt/spp/$index")
    ...
    private fun bond(replyHandler: ReplyHandler, pin: String) {
        ...
        val receiver = BondReceiver(context, deviceWrapper, replyHandler, pin) { device, state ->
